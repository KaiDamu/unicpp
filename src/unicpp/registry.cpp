...
dfa ER RegKey::_Get(ArrSbo<U1, 512>& buf, cx CH* name) cx
{
...
dfa HD RegKey::Hdl() cx
{
    ret m_hdl;
}
dfa ER RegKey::GetU4(U4& val, cx CH* name) cx
{
    val = 0;
    ArrSbo<U1, 512> buf;
...
dfa ER RegKey::GetStr(std::wstring& val, cx CH* name) cx
{
    val.clear();
    ArrSbo<U1, 512> buf;
    ifep(tx->_Get(buf, name));
...
dfa ER RegKey::SetU4(cx CH* name, U4 val) cx
{
    UNICODE_STRING_ str(name);
...
dfa ER RegKey::SetStr(cx CH* name, cx CH* val) cx
{
    UNICODE_STRING_ str(name);
...
dfa ER RegKey::Open(cx CH* path, U4 access, U4 options, HD hdlRoot)
{
    ifu (m_hdl != NUL)
...
dfa BO RegKeyIsExist(cx CH* pathBase, cx std::span<FNV1A64L>& subKeyList, HD hdlRoot)
{
    RegKey regKey;
