...
dfa U8 NetAdrV4::ToU8() cx
{
    ret (U8(ip) << sizb(port)) | U8(port);
}
dfa NetAdrV4 NetAdrV4::ToNoPort() cx
{
    NetAdrV4 result;
    result.ip = ip;
    result.port = 0;
...
dfa NT NetAdrV4::ToStr(CS* out) cx
{
    AU cur = out;
    ite (i, i < 4)
    {
...
dfa std::string NetAdrV4::ToStr() cx
{
    CS str[32];
    tx->ToStr(str);
    ret std::string(str);
}
dfa BO NetAdrV4::operator==(cx NetAdrV4& other) cx
{
...
dfa NetAdrV4::NetAdrV4(U4 ip, U2 port) : ip(ip), port(port)
{
}
dfa NetAdrV4::NetAdrV4(U1 ip1, U1 ip2, U1 ip3, U1 ip4, U2 port)
{
    tx->ip = U4(ip4) << 24 | U4(ip3) << 16 | U4(ip2) << 8 | U4(ip1);
    tx->port = RevByte(port);
}

dfa BO SockTcp::IsValid() cx
{
    ret m_hdl != INVALID_SOCKET;
}
dfa BO SockTcp::IsConnected() cx
{
    ret m_isConnected;
}
dfa HD SockTcp::Hdl() cx
{
    ret HD(m_hdl);
}
dfa ER SockTcp::New()
...
dfa ER SockTcp::OpenSrv(U2 port) cx
{
    ifu (m_hdl == INVALID_SOCKET)
        rete(ErrVal::NO_INIT);
    SOCKADDR_IN adrInfo = {};
...
dfa ER SockTcp::Accept(SockTcp& sockCli, NetAdrV4& adr)
{
    adr.Clr();
    ifu (m_hdl == INVALID_SOCKET)
        rete(ErrVal::NO_INIT);
...
dfa ER SockTcp::Write(CXGA buf, SI size) cx
{
    ifu (m_hdl == INVALID_SOCKET)
        rete(ErrVal::NO_INIT);
...
dfa ER SockTcp::Write(cx std::span<cx std::span<cx U1>>& bufList) cx
{
    ifu (m_hdl == INVALID_SOCKET)
        rete(ErrVal::NO_INIT);
...
dfa ER SockTcp::Read(GA buf, SI& sizeResult, SI sizeMax, SI sizeMin) cx
{
    sizeResult = 0;
    ifu (m_hdl == INVALID_SOCKET)
        rete(ErrVal::NO_INIT);
...
dfa NT SockTcp::Move(SockTcp& src)
{
    tx->Del();
    m_hdl = src.m_hdl;
...
dfa SockTcp::SockTcp() : m_hdl(INVALID_SOCKET), m_isConnected(NO)
{
