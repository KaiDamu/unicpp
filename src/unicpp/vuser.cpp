...
dfa VuserInput::TimeCurve::Param::Param()
{
    MemSet(tx, 0, siz(*tx));
}

dfa VuserInput::TimeCurve::TimeCurve()
{
    type = Type::LINEAR;
}

dfa VuserInput::PosCurve::Param::Param()
{
    MemSet(tx, 0, siz(*tx));
}

dfa VuserInput::PosCurve::PosCurve()
{
    type = Type::LINEAR;
}

dfa VuserInput::Cfg::Key::Key()
{
...
dfa NT VuserInput::Cfg::InitAdam()
{
    F4 scnDiagonal;
...
dfa VuserInput::Cfg::Cfg()
{
    tx->InitAdam();
}

dfa VuserInput::State::Key::Key()
{
}

dfa VuserInput::State::Cur::Cur()
{
    pos = Pos2<F4>(0.0f, 0.0f);
}

dfa VuserInput::State::State()
{
}

dfa VuserInput::Cfg& VuserInput::CfgGet()
{
    ret m_cfg;
}

dfa NT VuserInput::_CfgKeyVariApply(Cfg::Key& dst, cx Cfg::Key& src)
{
    dst = src;
...
dfa NT VuserInput::_CfgCurVariApply(Cfg::Cur& dst, cx Cfg::Cur& src)
{
    dst = src;
...
dfa ER VuserInput::KeyPressDown(InputKey key)
{
    Cfg::Key cfgKey;
...
dfa ER VuserInput::KeyPressUp(InputKey key)
{
    Cfg::Key cfgKey;
    tx->_CfgKeyVariApply(cfgKey, m_cfg.key);
...
dfa ER VuserInput::CurPosMove(cx Pos2<F4>& pos, cx Pos2<F4>& posVari)
{
    // get source position
    Pos2<F4> srcPos;
    ife (tx->_CurPosGet(srcPos))
