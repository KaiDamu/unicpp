...
public:
	...
	dfa NT Add(cx T1& dat) {
		ifu (tx->Len() + 1 > m_arr.Cap()) m_arr.Resize(SI(F8(tx->Len() + 1) * DSTACK_ADD_ALLOC_SCALE), tx->Len());
		m_arr.Write(dat);
	}
	dfa NT Add(cx T1* dat, SI cnt) {
		ifu (tx->Len() + cnt > m_arr.Cap()) m_arr.Resize(SI(F8(tx->Len() + 1) * DSTACK_ADD_ALLOC_SCALE), tx->Len());
		m_arr.Write(dat, cnt);
	}
