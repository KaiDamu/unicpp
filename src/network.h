...
dfa ER NetInit() {
	...
	if (isInitDone) rets;
	WSADATA wsaData;
	if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) rete(ERR_NET);
	isInitDone = YES;
	rets;
}
dfa NT NetFree() {
...
dfa ER DomainToIp(U4& ip, cx CS* domain) {
	...
	ADDRINFOA* result;
	ifu (getaddrinfo(domain, NUL, &hints, &result) != 0) rete(ERR_NET);
...
dfa ER StrToNetAdrDomain(NetAdr& netAdr, cx string_view& str) {
	...
	if (!strDomain.empty()) {
		U4 ip;
		ife (DomainToIp(ip, (divPos == string::npos) ? strDomain.data() : string(strDomain, 0, divPos).c_str())) retep;
		netAdr.ip = ip;
	}
	if (!strPort.empty()) {
		netAdr.port = StrToPort(strPort.data());
