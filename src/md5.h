...
public:
	...
	public:
		...
		dfa U4* B4() { ret b4; }
		dfa U8* B8() { ret b8; }
		dfa cx U1* B1() cx { ret b1; }
		dfa cx U2* B2() cx { ret b2; }
		dfa cx U4* B4() cx { ret b4; }
		dfa cx U8* B8() cx { ret b8; }
	public:
	...
	public:
		...
		dfa NT operator = (cx U1* val) { MemSet(tx, val, siz(*tx)); }
		dfa operator U1* () { ret b1; }
		dfa operator U2* () { ret b2; }
		dfa operator U4* () { ret b4; }
		dfa operator U8* () { ret b8; }
		dfa BO operator == (cx Val& val) cx { ret MemCmp(tx, val.B1(), siz(*tx)) == 0; }
		dfa BO operator != (cx Val& val) cx { ret MemCmp(tx, val.B1(), siz(*tx)) != 0; }
		dfa BO operator <  (cx Val& val) cx { ret MemCmp(tx, val.B1(), siz(*tx)) <  0; }
		dfa BO operator >  (cx Val& val) cx { ret MemCmp(tx, val.B1(), siz(*tx)) >  0; }
		dfa BO operator <= (cx Val& val) cx { ret MemCmp(tx, val.B1(), siz(*tx)) <= 0; }
...
public:
	...
	static dfa ER GetFile(Val& val, cx CH* path) {
		File f;
		ife (f.OpenRead(path)) retep;
		Md5::Gen md5;
		Arr<U1> buf(FILE_READ_SIZE_HIGH);
		SI readSize = 0;
		do {
			ife (f.Read(buf, buf.Cap(), readSize)) retep;
	...
	static dfa ER GetCsstrFile(CS* str, cx CH* path) {
		Val val;
		ife (Md5::GetFile(val, path)) retep;
		val.GetCsstr(str);
		rets;
	}
};
