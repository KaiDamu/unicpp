...
dfa cx CH* _RegKeySplitSub(cx CH* path, HKEY& key) {
	ifu (StrCmp(path, L"HKEY_", 5) != 0) ret NUL;
	path += 5;
...
dfa ER RegDirCreate(cx CH* path) {
	...
	HKEY subKeyHdl = NUL;
	ifu (RegCreateKeyExW(key, subKey, 0, NUL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NUL, &subKeyHdl, NUL) != ERROR_SUCCESS) rete(ERR_REG);
...
dfa ER RegDirCpy(cx CH* dst, cx CH* src) {
	...
	ifu (subKeySrc == NUL) rete(ERR_REG);
	HKEY keyDst;
	cx CH* subKeyDst = _RegKeySplitSub(dst, keyDst);
	ifu (subKeyDst == NUL) rete(ERR_REG);
	ifu (RegCopyTreeW(keySrc, subKeySrc, keyDst) != ERROR_SUCCESS) rete(ERR_REG);
	rets;
}
dfa ER RegDirDel(cx CH* path) {
	...
	cx CH* subKey = _RegKeySplitSub(path, key);
	ifu (subKey == NUL) rete(ERR_REG);
	ifu (RegDeleteTreeW(key, subKey) != ERROR_SUCCESS) rete(ERR_REG);
	rets;
}
...
dfa ER RegValSetU4(cx CH* path, U4 val) {
	...
	*name++ = '\0';
	...
	HKEY subKeyHdl;
	ifu (RegOpenKeyExW(key, subKey, 0, KEY_WRITE, &subKeyHdl) != ERROR_SUCCESS) rete(ERR_REG);
	ifu (RegSetValueExW(subKeyHdl, name, 0, REG_DWORD, (BYTE*)&val, siz(val)) != ERROR_SUCCESS) rete(ERR_REG);
	ifu (RegCloseKey(subKeyHdl) != ERROR_SUCCESS) rete(ERR_REG);
	rets;
}
...
dfa ER RegValDel(cx CH* path) {
	...
	cx CH* subKey = _RegKeySplitSub(path, key);
	ifu (subKey == NUL) rete(ERR_REG);
	ifu (RegDeleteValueW(key, subKey) != ERROR_SUCCESS) rete(ERR_REG);
	rets;
}
